...
from src.config import DATA_DIR, DEFAULT_TOP_N
...
def user_interaction() -> None:
    ...
    try:
        ...
        experience = (
            input('Стаж: введите значение "От" (Число. Иное - пропустить): ').replace(" ", "").replace(",", ".")
        )
        if experience:
            vacancies = VacancyAnalyzer(vacancies).filter_by_experience(experience)

        # 4. Фильтрация по зарплате
        salary_input = input("Введите диапазон зарплат (например, '100000-150000', Enter - пропустить): ").replace(
            " ", ""
        )
        if salary_input:
        ...
        input_top_n = input(
            f"Введите Топ-N вакансий по зарплате (число, Enter - вывести {DEFAULT_TOP_N} вакансий): "
        ).replace(" ", "")
