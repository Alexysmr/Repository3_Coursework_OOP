...
from src.config import setup_logging, DEFAULT_TOP_N, MAX_TOP_N
...
class Vacancy:
    ...
    def __init__(self, id_number: str, search_query: str, name: str, url: str, salary: any,
                 description: str, experience: str):
        self.id = id_number
    ...
    def _validate_salary(salary: dict) -> dict:
        ...
        if not salary or (salary.get('from') is None and salary.get('to') is None) or (
                salary.get('from') is None and not salary.get('to')) or (
                not salary.get('from') and salary.get('to') is None):
    ...
    def cast_to_object_list(hh_data: list[dict], search_query: str) -> list:
        ...
        vacancies = []
        for i in hh_data:
            vacancy = Vacancy(i["id"], search_query, i["name"], i["url"], i["salary"],
                              i["snippet"]["requirement"], i["experience"]["name"])
    ...
    def __repr__(self):
        ...
        return (f"Vacancy {self.search_query}\n"
    ...
    def to_dict(self) -> dict:
        ...
        return {'id': self.id, 'name': self.name, 'url': self.url, 'salary': self.salary,
                'description': self.description, 'experience': self.experience}
...
class VacancyAnalyzer:
    ...
    def filter_by_experience(self, experience: str | int) -> list[Vacancy]:
        """Фильтрация по стажу с поддержкой дробных значений"""
        logger.info(f"Фильтрация по стажу {experience} лет")
        experience_float = abs(float(experience)) if (str(
            experience).replace(".", "", 1).isdigit()) else 0.0  # Защита от формата "1.2.3"
        ranges = ((0.0, 1.0, "Нет опыта"),
    ...
    def get_top_n(self, input_top_n: int | str) -> list[Vacancy]:
    ...
    def filter_by_salary(self, salary_input: str) -> list[Vacancy] | None:
        ...
        try:
            ...
            if len(salary_range) >= 2:
                ...
                return [v for v in self.vacancies if
                        min_s <= v.salary["from"] <= int(min_s * 1.2) and v.salary["to"] <= max_s]
            return []
