...
from src.config import setup_logging, DATA_DIR, CACHE_EXPIRE_HOURS

modul_name = os.path.basename(__file__)
logger = setup_logging(modul_name)


def check_exist_data(search_query: str, file_path: Path | None = None, filename: str | None = None):
    ...
    if (data_file.exists() and ((datetime.now() - datetime.fromtimestamp(data_file.stat().st_mtime)).seconds / 3600)
            < CACHE_EXPIRE_HOURS):
        try:
